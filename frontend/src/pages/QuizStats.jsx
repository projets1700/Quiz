/**
 * Page Statistiques Globales d'un quiz : agrégation de toutes les sessions.
 * - Nombre de participants : somme sur toutes les sessions
 * - Autres indicateurs : moyennes/sommes sur toutes les sessions
 */
...
function QuizStats() {
  ...
  if (error) return <div className="quiz-stats-page"><div className="quiz-stats-error">{error}</div><Link to="/quizzes">Retour aux quiz</Link></div>;
  if (!data) return null;
  ...
  const handleSaveChart = () => {
    const section = document.querySelector('.quiz-stats-graph-section');
    if (!section) return;
    // Ouvrir une nouvelle fenêtre avec un résumé texte pour "sauvegarde" (export réel possible avec lib dédiée)
    const text = `Statistiques Globales - ${quiz.title}\nVue: ${CHART_VIEWS.find((v) => v.id === chartView)?.label ?? chartView}\n`;
    const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `stats-${quiz.title.replace(/\s+/g, '-')}.txt`;
    a.click();
    ...
    <div className="quiz-stats-page">
      ...
      <main className="quiz-stats-main">
        <section className="quiz-stats-section quiz-stats-graph-section">
          ...
          <p className="quiz-stats-global-note">Agrégation de toutes les sessions : participants (somme), réussite et temps moyen (moyennes).</p>
        ...
        {rankingEnabled && ranking.length > 0 && (
          <section className="quiz-stats-section quiz-stats-ranking">
            <h3 className="quiz-stats-ranking-title">Classement du dernier quiz</h3>
